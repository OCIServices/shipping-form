class OfficesController < ApplicationController
  ...
  # GET /offices/query/%query%.json
  def query
    @offices = Office.where('lower(name) like ?', "%#{params[:name]}%")
#    puts @offices.inspect
  ...
  private
    ...
    def office_params
      params.require(:office).permit(:address1, :address2, :address3, :city, :state, :zip, :country, :name)
    end
end
